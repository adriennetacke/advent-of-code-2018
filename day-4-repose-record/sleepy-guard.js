const findLaziestGuard = (input) => {
  ...
  let laziestGuardId = Object
    ...
    .find(x => Object
      .values(sleepMinutes[x])
      .reduce((a, b) => a + b, 0) === mostMinutesSlept);
  
  let sleepiestMinute = Object
    .values(sleepMinutes[laziestGuardId])
    .sort((a, b) => b - a)[0];
  
  let sleepiestMinuteId = Object
  .keys(sleepMinutes[laziestGuardId])
  .filter(x => sleepMinutes[laziestGuardId][x] === sleepiestMinute)[0];

  return +laziestGuardId * sleepiestMinuteId;
};

module.exports = findLaziestGuard;